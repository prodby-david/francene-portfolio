...
interface HairMakeupSectionProps {
  onBookService: (serviceTitle: string, estimatedCost?: number) => void;
  onCustomize: (serviceId: string) => void;
}

export function HairMakeupSection({ onBookService, onCustomize }: HairMakeupSectionProps) {
  const pkg = SERVICE_PACKAGES.find((p) => p.id === 'hair-and-makeup');
  if (!pkg) return null;

  return (
    <section id="hair-and-makeup" className="bg-[#faf7f4] py-20 px-6 sm:px-10 lg:px-20 border-t border-[#c5b3a6]/25">
        ...
        >
          <div className="max-w-2xl">
            <div className="text-xs uppercase tracking-[0.2em] text-[#6a5c51] font-semibold mb-3">
              <span>Service 01 &bull; Beauty &amp; Hair Artistry</span>
            </div>
            ...
            <p className="text-sm sm:text-base text-[#484740] mt-4 leading-relaxed">
              Professional hair styling and long-lasting makeup designed for weddings, debuts, pre-nup shoots, and special events. We craft glowing, camera-ready looks tailored to your individual style.
            </p>
          </div>
